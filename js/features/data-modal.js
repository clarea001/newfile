  ...
  <div class="dm3-banner">
    ...
    <div class="dm3-stats-row">
      <div class="dm3-stat-chip">
        ...
        <div class="dm3-stat-chip-lbl">文字与配置</div>
      </div>      
      <div class="dm3-stat-chip">
        <div class="dm3-stat-chip-num" id="dm3-stat-media">—</div>
        <div class="dm3-stat-chip-lbl">图片与媒体</div>
      </div>
      <div class="dm3-stat-chip">
        <div class="dm3-stat-chip-num" id="dm3-stat-cfg">—</div>
        <div class="dm3-stat-chip-lbl">本地缓存</div>
      </div>
...
async function updateStats(forceRecalc = false) {
    try {
        ...
        // 🌟 根治核心：精准计算新架构的三个纯净箱子
        // ==========================================
        let appDataSize = 0;   // 箱子1：APP_DATA (纯文本配置)
        let appMediaSize = 0;  // 箱子2：APP_MEDIA (图片媒体)
        let lsSize = 0;        // 箱子3：localStorage (本地小缓存)

        // 1. 精准称重 APP_DATA 和 APP_MEDIA
        try {
            // 直接拿新架构的两大核心包裹
            const dataPackage = await localforage.getItem('APP_DATA') || {};
            const mediaPackage = await localforage.getItem('APP_MEDIA') || {};

            // 箱子1：纯文字/配置数据
            appDataSize = new Blob([JSON.stringify(dataPackage)]).size;

            // 箱子2：纯图片/媒体数据
            appMediaSize = new Blob([JSON.stringify(mediaPackage)]).size;

        } catch (e) {
            console.warn('[dm3] 读取新架构包裹失败:', e);
        }
        ...
        try {
            for (let i = 0; i < localStorage.length; i++) {
                ...
                // 每个字符在 localStorage 中占 2 字节 (UTF-16)
        ...
        // 3. 算出本站真正的“干干净净”总重量
        const totalRealSize = appDataSize + appMediaSize + lsSize;
        ...
        // 顶部总览进度条
        const pct = quota > 0 ? Math.min(100, (totalRealSize / quota) * 100) : 0;
        const sz = get('dm3-storage-size');
        if (sz) {
            sz.textContent = quota > 0 ? `${fmt(totalRealSize)} / ${fmt(quota)}` : `${fmt(totalRealSize)} (无法获取配额)`;
        }

        const bar = get('dm3-progress-fill');
        if (bar) {
            bar.style.width = pct + '%';
            // 根据真实占比变色：0-60% 绿色，60-85% 橙色警告，85%+ 红色危险
            if (pct > 85) {
                ...
                // 恢复默认的主题色渐变
                bar.style.background = 'linear-gradient(90deg, var(--accent-color), rgba(var(--accent-color-rgb,224,105,138),.55))';
            }
        }

        // 下方三个纯净数据卡片
        const m = get('dm3-stat-msgs');
        if (m) m.textContent = fmt(appDataSize); // 对应 APP_DATA
        
        const med = get('dm3-stat-media');
        if (med) med.textContent = fmt(appMediaSize); // 对应 APP_MEDIA
        
        const c = get('dm3-stat-cfg');
        if (c) c.textContent = fmt(lsSize); // 对应 localStorage

        console.log(`[dm3] 极简根治统计: APP_DATA=${fmt(appDataSize)}, APP_MEDIA=${fmt(appMediaSize)}, LS=${fmt(lsSize)}, 真实总占=${fmt(totalRealSize)} / 配额=${fmt(quota)}`);
    ...
    function bindExportImportEvents() {
      ...
      if (exportChatBtn) {
          exportChatBtn.addEventListener('click', () => {
              ...
              if (typeof exportChatHistory === 'function') ChatBackup.exportChatHistory(false); 
          });
      }
      ...
      if (exportAllBtn) {
          exportAllBtn.addEventListener('click', () => {
              ...
              if (typeof exportFullBackup === 'function') ChatBackup.exportFullBackup();
              else showNotification('功能暂不可用', 'error');
          });
      }
      // --- 3. 导入 (智能识别) ---
      ...
      const handleImportClick = () => {
          ...
          inp.onchange = e => {
              if (e.target.files[0]) {
                  // 调用新的智能导入函数
                  if (typeof importAnyBackup === 'function') ChatBackup.importAnyBackup(e.target.files[0]);
              }
          };
          inp.click();
      };
      ...
      // ================= 浏览器存储 FAQ 弹窗 (按需加载根治版) =================
      /*const faqTrigger = document.getElementById('dm3-open-faq');
      if (faqTrigger) {
        faqTrigger.addEventListener('click', () => {
          // 只有当用户真正点下这个按钮时，才去检查和创建弹窗
          let faqModal = document.getElementById('dm3-faq-modal');
          
          // 如果还没创建过，现造一个（整个网站生命周期内只会执行一次）
          if (!faqModal) {
              const faqList = [
              ...
              faqModal.id = 'dm3-faq-modal';
              faqModal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:99999;display:none;align-items:center;justify-content:center;padding:20px;';
              faqModal.innerHTML = `
              ...
              // 绑定关闭逻辑
              const closeModal = () => { faqModal.style.display = 'none'; };
